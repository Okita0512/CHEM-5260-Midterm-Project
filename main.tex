...
photochemical reactions~\cite{Nitzan}.  In its simplest idealization,
a single initially populated ``bright'' or \emph{doorway} state is coupled
to a dense manifold of background states; population leaks irreversibly out
of the doorway state on a timescale governed by the manifold density of
states and the coupling matrix elements.
...
In a true infinite continuum the exponential decay is exact and permanent.
In any finite numerical simulation, however, the manifold is necessarily
discrete and bounded, so the continuum picture must emerge as an approximation
valid over a finite time window.  Understanding when and how the approximation
breaks down is essential for reliably applying the theory to real molecular
systems.
...
By varying the parameters $N$, $\Delta E$, $V_{1k}$, $V_{01}$, and the energy
offset $E_0-E_1$, we test these analytical predictions against the exact
dynamics and delineate the boundaries of the wide-band regime.  In
particular, we show how finite-size recurrences, narrow bandwidths, and
strong couplings drive systematic departures from exponential relaxation in
Model~A, while in Model~B they modify the damped two-level picture and the
crossover between monotonic transfer and damped Rabi oscillations.  We also
compare uniform, random-sign, and Gaussian bath couplings to separate
gauge-trivial sign disorder from genuine magnitude disorder.
...
\begin{equation}
    ...
    \label{eq:Ek}
\end{equation}
giving a level spacing $\epsilon = \Delta E/N$ and density of states
$\rho = N/\Delta E$.  Without loss of generality we set $E_1 = 0$.
...
\begin{equation}
    ...
    \label{eq:lambdaB}
\end{equation}
Therefore the relevant regimes are determined by the ratio $V_{01}/\Gamma$:
\begin{enumerate}
...
critically damped, or oscillatory.
...
\begin{figure}[htbp]
    ...
    (overdamped-to-underdamped crossover).}
    \label{fig:effectV}
\end{figure}
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:seqFlow}%
    ...
    (Eqs.~\ref{eq:lambdaB}--\ref{eq:damped_c0}).
...
subspace to a damped harmonic oscillator (Eq.~\ref{eq:damped_c0}), with the
bath setting $\Gamma$ and $V_{01}$ playing the role of the restoring frequency.
...
\begin{figure*}[htbp]
  ...
  \caption{\label{fig:resonance}%
    ...
    \textbf{(C)} Extracted population decay rate $\gamma_\mathrm{eff}$ vs\
...
\begin{figure*}[htbp]
  ...
  \caption{\label{fig:effectNB}%
    ...
    $N_{\rm ref}=200$ so that $NV_{1k}^{2}=0.5$ and
    ...
    (Sec.~\ref{sec:GF}, Eq.~\ref{eq:damped_c0}).
...
scaled to keep $\Gamma\approx0.314$ and $NV_{1k}^{2}=0.5$ fixed, and
...
relaxation in finite systems.  The wide-band approximation supplies the
bridge from finite Hamiltonians to irreversible-looking decay, while the
exact numerics make clear where that bridge holds and where it breaks down.
For Model~A this yields the standard FGR exponential regime and its
finite-size corrections; for Model~B it yields a damped two-level problem
whose monotonic, critical, and oscillatory transfer regimes are all observed
directly in the exact dynamics.

At the same time, the present study remains intentionally minimal.  The bath
is represented by a finite set of discrete levels and the analysis is
effectively restricted to zero-temperature relaxation, with no explicit
molecular vibrational coordinates such as harmonic oscillators included in
the Hamiltonian.  These simplifications make the origin and breakdown of the
wide-band approximation especially transparent, but they also limit the
direct chemical realism of the models.

A natural direction for future work is therefore to embed the present
doorway-state picture in more microscopic open-system models.  In
particular, it would be valuable to replace the discrete phenomenological
bath by an explicit oscillator environment, for example in the spirit of the
Caldeira--Leggett model~\cite{Caldeira_Leggett_1983,Leggett_1984,Weiss}, so that temperature effects, spectral densities,
memory kernels, and vibronic structure can all be treated on the same
footing.  Such extensions would provide a more complete connection between
the simple finite-dimensional models studied here and realistic molecular
relaxation dynamics.
