...
\begin{abstract}
We study quantum relaxation in two finite-dimensional doorway-state models
using Green's functions under the wide-band approximation and exact
diagonalization of the full Hermitian Hamiltonian.  Model~A contains a
single doorway state $\ket{1}$ coupled to a bath manifold, whereas Model~B
adds an upstream state $\ket{0}$ to produce sequential transfer
$\ket{0}\to\ket{1}\to\{\ket{k}\}$.  For Model~A, the analysis recovers the
Fermi's-golden-rule form $P_1(t)=e^{-\Gamma t}$ with
$\Gamma=2\pi |V|^2\rho$, and the numerics show that this regime is realized
when $\rho^{-1}\lesssim\Gamma\ll\Delta E$; outside this window, finite-size
effects, narrow bandwidth, and strong coupling produce oscillations and
Poincar\'e recurrences.  For Model~B, the same reduction yields an
effective damped two-level problem with overdamped, critical, and
underdamped regimes separated by $V_{01}=\Gamma/4$, together with a
Lorentzian detuning dependence.  Random-sign bath couplings are exactly
gauge-equivalent to uniform couplings, whereas Gaussian magnitude disorder
creates a distribution of realization-specific decay rates, so the
ensemble-averaged dynamics deviates from the single-rate FGR prediction and
decays more slowly at intermediate and long times.
\end{abstract}
...
self-energy methods~\cite{Nitzan,Weiss}, predicts exponential decay when the
manifold is broad and dense enough to act as a quasi-continuum.  The decay
rate is then given by Fermi's golden rule (FGR):
\begin{equation}
    \Gamma = 2\pi|V|^2\rho,
    \label{eq:FGR}
\end{equation}
...
units where $\hbar=1$.  This result relies on the \emph{wide-band
approximation}, which requires
\begin{equation}
    \rho^{-1} \lesssim \Gamma \ll \Delta E.
    \label{eq:wideband}
\end{equation}
When these inequalities fail, finite systems need not relax exponentially:
recurrences, persistent oscillations, and incomplete decay can all appear.
Thus irreversible Markovian relaxation is not a generic property of a
finite bath, but a continuum-limit behavior that must be justified.  A
related subtlety appears when the doorway--bath couplings are disordered.
Random-sign disorder can be removed by a gauge transformation on the bath
basis, but Gaussian magnitude disorder generates a distribution of effective
rates across realizations.  The disorder average therefore takes the form
$\langle e^{-\Gamma t}\rangle$, which generally differs from the mean-rate
FGR form $e^{-\langle\Gamma\rangle t}$ even when each realization is nearly
Markovian.
...
For Model~A we recover the standard wide-band result
$P_1(t)=e^{-\Gamma t}$.  For Model~B the same approximation reduces the
problem to an effective damped two-level system for
$\{\ket{0},\ket{1}\}$~\cite{CohenTannoudji1997}, with overdamped, critical,
and underdamped regimes separated by $V_{01}=\Gamma/4$ and a Lorentzian
dependence on the detuning $E_0-E_1$.  Exact diagonalization of the full
finite Hamiltonian is then used to test these predictions as $N$,
$\Delta E$, $V_{1k}$, $V_{01}$, and $E_0-E_1$ are varied, and to compare
uniform, random-sign, and Gaussian couplings.  Our central goals are to
identify when a finite bath reproduces FGR relaxation, how the wide-band
approximation breaks down, how static disorder modifies the relation between
single-realization and ensemble dynamics, and how the upstream state alters
...
seed-to-seed fluctuations in the coupling magnitudes and therefore in the
realization-specific decay rates $\Gamma_r\propto \sum_k |V_{1k}|^2$.
Each realization still decays approximately as
$P_1^{(r)}(t)\approx e^{-\Gamma_r t}$ in the wide-band regime, but the
ensemble average becomes
$\langle P_1(t)\rangle=\langle e^{-\Gamma_r t}\rangle\neq
e^{-\langle\Gamma\rangle t}$ whenever $\mathrm{Var}(\Gamma_r)\neq 0$.
The deviation from FGR is therefore physical rather than numerical: static
Gaussian disorder generates rate heterogeneity, so the averaged relaxation
is a superposition of exponentials instead of a single exponential.  In
fact, for $t\ge 0$ Jensen's inequality gives
\begin{equation}
    \langle e^{-\Gamma_r t}\rangle \ge e^{-t\langle\Gamma\rangle},
\end{equation}
because $e^{-x}$ is strictly convex.  This structure is mathematically
parallel to the Jarzynski relation, where an exponential average is also
controlled by fluctuations rather than by the mean alone.  In the present
context the exponential weighting favors rare low-$\Gamma$ realizations:
fast trajectories decay away quickly and cease to contribute, whereas
slow trajectories dominate the ensemble average at intermediate and long
times.

The inequality is saturated only if $\Gamma_r$ is nonfluctuating across the
ensemble, or trivially at $t=0$.  This explains why random-sign disorder,
for which $|V_{1k}|^2=V^2$ exactly, reproduces the uniform-coupling result
for all times.  For Gaussian magnitude disorder the inequality is strict for
any $t>0$.  In the weak-disorder or short-time regime it is nearly
saturated: writing $\Gamma_r=\bar{\Gamma}+\delta\Gamma$ with
$\langle\delta\Gamma\rangle=0$, a cumulant expansion gives
\begin{equation}
    \ln\langle e^{-\Gamma_r t}\rangle
    = -\bar{\Gamma} t + \frac{t^2}{2}\mathrm{Var}(\Gamma_r)
    - \frac{t^3}{6}\kappa_3(\Gamma_r) + \cdots,
\end{equation}
so the leading departure from the mean-rate FGR form is controlled by
$t^2\mathrm{Var}(\Gamma_r)/2$.  For Gaussian couplings,
$\Gamma_r\propto N^{-1}\sum_k V_{1k}^2$, so the relative variance of
$\Gamma_r$ scales as $N^{-1}$; hence the inequality becomes nearly sharp in
the self-averaging large-$N$ limit, but at long times even small residual
fluctuations are amplified because the Laplace transform is dominated by the
low-rate tail of the distribution.
...
The Gaussian ensemble mean remains close to the FGR trend at short times,
but on a logarithmic scale it bends away from a perfect straight line
because it is the Laplace transform of the distribution of $\Gamma_r$.
Realizations with larger $\langle V_{1k}^2\rangle$ decay faster, whereas
slower realizations dominate the long-time tail, producing a decay that is
slightly slower than the single-rate FGR prediction and smearing the revival
structure.  By contrast, random-sign disorder leaves $|V_{1k}|^2=V^2$
exactly for every $k$, so all realizations share the same $\Gamma$ and the
ensemble average coincides with the uniform result.  All three schemes
satisfy $\langle\Gamma\rangle = 2\pi V_{\mathrm{rms}}^2\rho$ to within
statistical uncertainty, and the residual deviation decreases as $N$
increases and the fluctuations of $\Gamma_r$ self-average away.
...
\begin{figure*}[htbp]
  ...
  \caption{\label{fig:disorderB}%
    ...
    \emph{Green (Gaussian $V_{1k}$):} genuine disorder shifts the ensemble
    mean away from the uniform/random-sign and wide-band reference dynamics,
    and broadens $P_1(t)$ near its transient peak; the variance is reflected
...
the variance grows over the observation window, and the ensemble-averaged
dynamics deviates slightly from both the uniform/random-sign result and the
corresponding wide-band single-rate expectation.
...
dynamics remains close to the corresponding wide-band prediction, but the
ensemble average deviates systematically from a single FGR exponential
because Gaussian magnitude disorder produces a distribution of
realization-specific rates $\Gamma_r$.  Thus
$\langle e^{-\Gamma_r t}\rangle \ge e^{-\langle\Gamma\rangle t}$ by
Jensen's inequality, with equality only when $\Gamma_r$ does not fluctuate
across realizations.  The leading weak-disorder correction is
$t^2\mathrm{Var}(\Gamma_r)/2$, while at long times the slowly decaying
realizations dominate the average.  The resulting sample-to-sample
fluctuations broaden the envelopes and are most visible in the transient
doorway population and in late-time recurrence features.
\end{enumerate}
...
observation.  Static Gaussian disorder adds a second, distinct mechanism for
departing from the simplest FGR picture: even when each realization is
locally consistent with Markovian decay, averaging over the resulting
distribution of rates produces a non-exponential ensemble response in which
the slow tail is enhanced.
